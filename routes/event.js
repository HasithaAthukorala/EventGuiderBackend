...
router.post('/register', auth, function (req, res, next) {
    ...
    if (Object.keys(errorList).length !== 0) {
        ...
        eventController.registerForEvent(req.user.id, req.body.event_id, req.body.tag, function (status, msg) {
...
router.post('/users', function (req, res, next) {
    if (req.user.name !== 'admin') {
        ...
        if (Object.keys(errorList).length !== 0) {
            ...
            eventController.getEventUsers(req.event_id, function (status, result) {
                res.status(status).json(result);
            });
        }
    }
